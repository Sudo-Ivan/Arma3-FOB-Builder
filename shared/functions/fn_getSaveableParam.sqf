...
private _saveKey = "CWS_BUILDER_" + (toUpper worldName) + "_SAVE_PARAMS";
private _value = nil;
...
switch (_action) do {
    ...
    case 0: {
        ...
        if(isNil "_savedParams") then {
            ["Creating new save data", "PARAM"] call shared_fnc_log;
    ...
    case 1: {
        private _savedParams = profileNamespace getVariable _saveKey;
        if(isNil "_savedParams") then {
            ["Param save data is corrupted, can't load!", "PARAM"] call shared_fnc_log;
            // Fix param save data
            profileNamespace setVariable [_saveKey, []];
            [format ["No saved value for param: %1, fetching value.", _paramName], "PARAM"] call shared_fnc_log;
            _value = [_paramName, _defaultValue] call bis_fnc_getParamValue;
        } else {
            private _singleParam = (_savedParams select {(_x select 0) == _paramName}) select 0;
            if(isNil "_singleParam") then {
    ...
    // Get param
    default {
        [format ["Fetching selected value for param: %1", _paramName], "PARAM"] call shared_fnc_log;
        _value = [_paramName, _defaultValue] call bis_fnc_getParamValue;
    };
};
